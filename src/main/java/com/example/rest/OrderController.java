...
public class OrderController {
    ...
    ResponseEntity<RepresentationModel> cancel(@PathVariable Long id) {
        ...
        If it’s not a valid state, it returns a Spring HATEOAS
        VndError, a hypermedia-supporting error container.

        If the transition is indeed valid, it transitions the Order to CANCELLED.
         */
    ...
    ResponseEntity<RepresentationModel> complete(@PathVariable Long id) {
        ...
        return ResponseEntity
                .status(HttpStatus.METHOD_NOT_ALLOWED)
                .body(new VndErrors.VndError("Method not allowed", "You cant't complete and order that is in the " + order.getStatus() + " status"));
    }
}
